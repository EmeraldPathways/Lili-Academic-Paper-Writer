...
// The original prompt logic is moved here, to the secure server
const getFullPrompt = (draft, style) => {
...
app.post('/api/generate', async (req, res) => {
  try {
    const { draft, style } = req.body;

    if (!draft || !style) {
      return res.status(400).json({ error: 'Draft and style are required.' });
    }
...
// Fallback to serve index.html for any other request (for client-side routing)
app.get('*', (req, res) => {
